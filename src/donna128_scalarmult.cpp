...
static void force_inline fsum(uint64_t *output, const uint64_t *in)
{
    output[0] += in[0];
    output[1] += in[1];
    output[2] += in[2];
 ...
 */
static void force_inline fdifference_backwards(felem out, const felem in)
{
    /* 152 is 19 << 3 */
...
static void force_inline fscalar_product(felem output, const felem in, const uint64_t scalar)
{
    uint128_t a;
 ...
 */
static void force_inline fmul(felem output, const felem in2, const felem in)
{
    uint128_t t[5];
    ...
    t[4] = ((uint128_t)r0) * s4 + ((uint128_t)r4) * s0 + ((uint128_t)r3) * s1 + ((uint128_t)r1) * s3
           + ((uint128_t)r2) * s2;
...
static void force_inline fsquare_times(felem output, const felem in, uint64_t count)
{
    uint128_t t[5];
...
static void fexpand(uint64_t *output, const uint8_t *in)
{
...
static void fmonty(
    uint64_t *x2,
    uint64_t *z2, /* output 2Q */
    uint64_t *x3,
    uint64_t *z3, /* output Q + Q' */
    uint64_t *x,
    uint64_t *z, /* input Q */
    uint64_t *xprime,
    uint64_t *zprime, /* input Q' */
...
// information.
// -----------------------------------------------------------------------------
static void swap_conditional(uint64_t a[5], uint64_t b[5], uint64_t iswap)
{
    unsigned i;
 ...
 */
static void cmult(uint64_t *resultx, uint64_t *resultz, const uint8_t *n, const uint64_t *q)
{
...
int donna128_scalarmult(uint8_t *mypublic, const uint8_t *secret, const uint8_t *basepoint)
{
